...
/* 2026-09-03 Leo：「查任意股風格不一致，請改成跟燈號風格一致」。
   ...
   （--surface #080E1A / --line #16304A）並排一看就是兩個網站。
...
NOTE = (
    ...
    '圖表：K線用雙重颱風三色、SuperTrend 黃多紫空，<b>點一下圖</b>才啟用滾輪縮放'
    '（預設滾輪是捲頁面），拖曳平移，四張圖連動。<br>'
    # 🔴 2026-09-08 Leo：「這個什麼意思? 停損還是有畫圖」——
    #    這段是 9/7 加那幾條線**之前**寫的，加完沒有回頭改，
    #    變成頁面自己說「沒有停損線」但圖上明明畫著 769.8 停損4倍。
    #    ⭐ **加功能要順手查有沒有哪段說明在講「我們沒有這個」。**
    #       過期的免責比沒有免責更糟——它會讓人懷疑其他敘述也不可信。
    '圖上四條線：<b>目標價</b>（yfinance <b>市場共識</b>，多家券商平均，'
    '<b>不是單一投顧的目標價</b>）、<b>20 日平均成本</b>、'
    '<b>SuperTrend</b>（＝老墨說的「3 倍停損」，實測數字一致）、'
    '<b>停損 4 倍</b>（雙重颱風 mult=4.0）。<br>'
    '⚠️ 仍然沒有的是<b>單一投顧的目標價</b>——那要有券商報告才有，'
    '本系統沒有通用資料源，不做假的欄位（有報告的個股看'
    '<b>整合報告</b>）。'
    '</div>')
...
def resolve(q):
    ...
    if not q:
        return []
    # 2026-09-20 Leo：查「6291」查不到，卻跳出「像公司名不是代號」的選單，
    # 列出6291.T（日股Airtech Japan）、62916.HK 這種跟台股6291完全無關的候選。
    # 根因：純數字/台股代號形狀的輸入本來就不是「公司名」，不該送進 yf.Search
    # 做模糊比對——那支 API 對一串數字會回一堆巧合命中的全球代號，命中越多反而
    # 越誤導。docstring 本來就寫了「輸入本來就是代號時回空清單」，但只有中文
    # 分支照做了，純數字/代號形狀的分支漏了這個判斷，直接掉進 yf.Search。
    # 這裡補上：長得像代號（純數字、可選字母尾碼、可選.TW/.TWO）就直接回空，
    # 讓上層 render() 走「查無資料」那條路——那才是真正的狀況（lookup() 本身
    # 抓資料失敗，通常是暫時性問題，不是代號打錯）。
    import re as _re_code
    if _re_code.match(r"^\d{4,6}[A-Z]?(\.TWO?)?$", q.upper()):
        return []
