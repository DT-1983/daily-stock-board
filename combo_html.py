...
# 2026-09-20 Leo（兩次澄清後的定案）：「預設進去就是戰情室的列表；本機離線時，**需要本機的功能**
# 不能使用（不是全部不能用）；列表備援還是需要」。
# → docs/combo.html 仍是完整列表（備援），但最上面有一段偵測腳本：探測本機 /ping（discord_bot.py，
#   只回 {up:true}），在線 → 立刻 location.replace 進戰情室（保留 ?ticker=）；
#   離線（逾時 2.5 秒或 fetch 失敗；電腦關機時 tunnel 回 502、沒有 CORS 標頭，fetch 會 reject）
#   → 顯示列表＋一條說明「哪些功能要本機開機」，查股框反灰。
# 判定前列表先隱藏（.gatehide），避免在線的人先看到一閃而過的備援列表。
# ⚠️ 家人也看得到這個網址：沒授權的裝置被轉進戰情室會看到「這台裝置還沒授權」頁，
#    不會外流任何數字（門檻在伺服端，見 lookup_page.gate）。
GATE_CSS = """
.gatehide{visibility:hidden}
.gatebar{margin:12px 0 4px;padding:10px 14px;border:1px solid var(--line);
 border-left:3px solid #EAB308;background:var(--card);font-size:13px;line-height:1.7;color:var(--muted)}
.gatebar b{color:var(--ink)}
.gatebar button{margin-left:8px;padding:2px 10px;border:1px solid var(--line);background:none;
 color:var(--cy,#22D3EE);font:inherit;font-size:12px;cursor:pointer}
.gatebar[hidden]{display:none}
.lkoff{opacity:.5;pointer-events:none}
"""
...
</script><noscript><style>.gatehide{visibility:visible!important}</style></noscript>""".replace("__PING__", PING_URL).replace("__ROOM__", ROOM_URL)

# 備援模式的查股框：外觀留著（讓人知道有這功能），但反灰不能用。
LOOKUP_BOX_OFF = (
    '<div class="lkbox lkoff"><input disabled placeholder="代號或名稱：2454 / 台積電 / COST">'
    '<button disabled>查燈號</button>'
...
def body_html(d, in_room=False, public=False):
    ...
    body = ['<div class="cbwrap' + (' gatehide' if public else '') + '" id="cbwrap">']
    body.append('<div class="cbtop"><div class="cbstat">'
                ...
                + (LOOKUP_BOX if in_room else (LOOKUP_BOX_OFF if public else ''))
                ...
                # 🔴 2026-09-20 Leo：「燈號跟進出燈號二邊跳來跳出有點錯亂」→ 三種呼叫方式：
                #    · in_room：戰情室列表模式（查股框可用）
                #    · public：docs/combo.html——偵測到本機在線會轉進戰情室，離線才看得到這份
    ...
    body.append('<div class="cbnote">'
                ...
                '這頁只管進場時機，不是停利建議。</div>')
    body.append("</div>")
    return (gatebar_html(d) if public else '') + NL.join(body)
...
def _page(d, public):
    ...
    return ("<!doctype html><html lang=\"zh-Hant\"><head><meta charset=\"utf-8\">"
            ...
            + chr(34) + "></script>"
            "<style>" + BASE_CSS + CSS + LOOKUP_CSS + _ti_css() + (GATE_CSS if public else "")
            + "</style></head><body>"
            # 偵測腳本放在 body 最前面：在線時要在列表下載完之前就轉走
            + (GATE_JS if public else "")
            # header 必須包在 .wrap 裡（首頁就是這樣做的），否則標題會貼齊視窗
            # 左緣、跟下面的內容對不齊——.wrap 才有 max-width:1100px + 置中。
            + "<div class=" + chr(34) + "wrap" + chr(34) + ">"
            + build(d, public=public) + "</div></body></html>")
...
def main():
    ...
    if a.no_obis:
        return 0
    try:
        snap = _page(d, public=False)
        os.makedirs(OBIS, exist_ok=True)
        io.open(os.path.join(OBIS, "COMBO打點.html"), "w",
                encoding="utf-8", newline=NL).write(snap)
        print(f"✅ 已存：{OBIS}（獨立快照，{len(snap):,} bytes）")
    except Exception as e:                              # noqa: BLE001
