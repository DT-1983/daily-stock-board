...

{
 ...
 "dilution_pct": "報告有提到未來可能稀釋股本的數字（GDR／可轉債／私募／庫藏股註銷回沖等）換算成的稀釋比例百分比數字，例「最多稀釋 8.43%」就填 8.43；沒有提到就填 null",
 "dilution_note": "報告怎麼描述這個稀釋（例：GDR全數轉換最多稀釋8.43%股本），照抄改繁中；沒提到填空字串"
}
...
- **報告可能是英文的（外資券商）**，欄位名稱會長這樣，一樣要抽出來：
  ...
  那幾頁一定要看。
- **`dilution_pct`／`dilution_note`**：只抓報告**自己揭露**的稀釋數字（GDR／可轉債／
  ECB／私募），不要自己去估算或猜；沒提到就是這份報告沒把潛在稀釋算進估值，
  這本身是一個訊號（不同券商對同一檔的揭露程度不同，是有意義的差異）。
"""
