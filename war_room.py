...
# 每位軍師自己的字數上限（2026-09-06 Leo：「軍師們還是有點囉嗦」）。
#
# ⚠️ 這是**第二次**處理囉嗦。9/4 那次的做法是在 prompt 裡加五條精簡規則
# ——加完之後 Leo 再看一次，還是覺得囉嗦。同一招再加更多條字沒有意義。
# ⭐ prompt 是**請求**，字數上限是**約束**。真正會逼出取捨的是後者：
#    650 字寫不下四段免責，只寫得下結論加兩個數字。
#
# 數字依角色該講的東西訂：風險官要的是「哪幾檔、為什麼」，最短；
...
def _unresolved_block(q):
    """問題看起來在問某一檔、但我們解析不出來時，**把這件事寫進材料**。

    回空字串＝這個問題本來就不是在問個股（例如「今天要注意什麼風險」），
    那就不需要這段。判斷刻意寬鬆：寧可多印一次警告，也不要漏掉一次代換。
    """
    import re as _re
    q = (q or "").strip()
    if not q or not _re.search(r"[0-9]{4,6}|[A-Za-z]{2,6}", q):
        return ""
    return ("【⚠️ 問題裡提到的個股：解析不到】\n"
            f"  Leo 的問題是「{q}」，但系統**沒有查到對應的股票資料**"
            "（可能是代號打錯、剛上市、或這一刻查價失敗）。\n"
            "  🔴 這種情況唯一正確的回答是「這一檔我查不到資料」。"
            "**不准**拿材料裡任何其他代號代替——即使它的名字只差一兩個字母。\n"
...
ROLES = {
    "仲達": {
        ...
        "persona": (
            ...
            "不是預測未來、也不是給進出場指令。\n"
            "說話方式（2026-09-06 Leo：「沒有軍師的風格特色」）：\n"
            "你話少、冷、只講壞消息。**先講最壞的那一檔**，再講第二嚴重的，講完就停。\n"
            "不鋪陳、不安慰、不給行動建議。看不出風險就說「今天沒有值得講的」，不要為了交差湊三件事。\n"
            "你是唯一會說「這件事我們沒有資料，所以我不知道」的人——這句話你要敢講。\n"
            "鐵律：\n"
    ...
    "孔明": {
        ...
        "persona": (
            ...
            "說話方式：\n"
            "你條理分明、兩面並陳，**先講判斷再講理由**，理由最多兩句。\n"
            "你最重要的一句話永遠是「什麼情況代表我錯了」——那句不能省，其他都可以省。\n"
            "鐵律：\n"
    ...
    "龐統": {
        ...
        "persona": (
            ...
            "不是下投資判斷。\n"
            "說話方式：\n"
            "你快、直、不客氣。沒用的材料就一句「這幾則跟這檔無關」帶過，不要逐則交代。\n"
            "你只講「發生了什麼、什麼時候」，不評價好壞——那是孔明的事。\n"
            "鐵律：\n"
    ...
    "陳壽": {
        ...
        "persona": (
            ...
            "以及（樣本夠時）Leo 自己的交易紀律。\n"
            "說話方式：\n"
            "你是史官：只記錄，不評價，不預測。**先給數字，再給一句話的觀察。**\n"
            "樣本不夠就寫「還不能下結論，目前 N 筆」，然後停——不要用個案講成趨勢。\n"
            "鐵律：\n"
...
📐 **排版規則（2026-09-04 Leo：「可以多一些圖示或段落嗎，現在有點難閱讀」）**：
這會貼在 Discord，讀的人多半在手機上。

- **燈號只有下判斷的人可以用**（孔明的兩個角度、仲達標風險嚴重度）。
  🟢 續抱／可買　🟡 觀望　🔴 考慮出場　⚪ 資料不足
  🔴 **龐統和陳壽不准用燈號開頭**——他們的職責裡明寫不下買賣判斷（龐統只給材料、陳壽只記錄），開頭掛一個 🟢 就等於下了判斷。
  2026-09-06 實際發生過：龐統開頭寫「🟢 續抱」、陳壽在一檔 4/4 多方的股票上寫「🔴」——燈號跟內容互相矛盾，因為他們本來就不該給燈號。
  ⭐ 這條格式規則原本寫「一律」，把兩位的鐵律蓋過去了：**全域格式規則會覆蓋角色設定，訂的時候要想清楚它對每個角色是不是都成立。**
- **每個大段落之間空一行**。連續五六行不換氣是現在最難讀的地方。
- **事實用 `・` 開頭，推論用 `→` 開頭**，一眼分得出哪句是資料、哪句是判讀。
  不要再寫「事實：」「推論：」這種標籤，符號就夠了。
- **數字前後留白**，例如 `現價 3,570`、`距停損線 +19.4%`。千分位要加逗號。
- 一行**不要超過 40 個字**，長句拆成兩行。
- 標題用 `**粗體**` 就好，不要用 `#` 標題（Discord 上會變得很大很吵）。
- 最後如果有免責或限制，用 `⚠️` 起頭**單獨一行**，不要混在段落裡。
...
def council_roles(question=""):
    ...
    """
    # 2026-09-06 Leo：「軍議可以讓龐統、孔明先跑再跑仲達、陳壽」。
    #
    # 順序寫成**一份 ORDER**，不再各自手寫兩個 list——手寫兩份的話，
    # 之後改順序一定會有一份沒改到（同一個形狀這個專案踩過三次）。
    # 這個順序本身有意義，不是排版：
    #   龐統（發生了什麼）→ 孔明（怎麼看）→ 仲達（有什麼風險）→ 陳壽（我們過去準不準）
    # 材料 → 判斷 → 風險 → 回顧，先看到事實再看到判斷，讀的人跟得上。
    #
    # ⚠️ 孔明**沒有指定個股就不能跑**：他的職責是「對一檔給兩個獨立角度」，
    #    沒有那一檔，材料是空的，他只會講一段沒有對象的通論。這不是我省略他，
    #    是這個問題形狀他答不了。
    ORDER = ["龐統", "孔明", "仲達", "陳壽"]
...
def ask(role, question=None, limit=None, prior=None):
    ...
    if not named:
        ...
        # 9/4「高力→HIG」之後我在註解裡寫了「解析不到時材料裡明寫這件事」，
        # **但程式從來沒做這件事**——`if named:` 解析失敗就什麼都不加，
        # 材料裡完全看不出「Leo 問了一檔我們沒查到的股票」。於是模型看到問題裡
        # 有 ONON、材料裡有 ON（而且 ON 剛好兩條出場訊號都成立），就拿 ON 當答案。
        # ⭐ **註解寫了不等於程式做了**——我以為補過這個洞，其實只補了 prompt 那半邊。
    ...
    for _ in range(3):
        ...
        if not bad:
            # 超字數就重寫一次，**不要硬切**。
            # 原本是 `txt[:limit]`——句子被切在一半，讀起來像斷線，
            # 而且被砍掉的通常是結尾的免責，看起來反而更肯定。
