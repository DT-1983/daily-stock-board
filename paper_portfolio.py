...
調倉頻率（2026-09-03 更正：以下時間查自實際 workflow 的 cron，不是憑印象）：
  · 產業鏈全 / 巴菲特價值 / 7 鏈明細 → `weekly-screen.yml` cron 0 0 * * 6
    ＝**台灣週六 08:00** 隨守備清單重篩調倉（原本這裡寫「每週日」是錯的）
  · 產業鏈+趨勢 / 進出燈號 / 淨值 → `portfolio-nav.yml` cron 0 1 * * 1-6
    ＝**台灣週一~週六 09:00**
