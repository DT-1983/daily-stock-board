...
#
# ⚠️ WARN_N 取 7 的理由（Leo 說「改用小 N」，具體數字我照回測挑最不武斷的那個）：
#   N=1~7 的最大回檔**完全一樣**（-10.42%），差別只在進出次數（19 → 5 次）
#   與報酬（+85% → +104%）。N=7 在這一段裡進出最少、報酬最高，
#   而且它是「小 N 這個區間」的邊界值，不是從中間挑一個好看的點。
# ⚠️ 這個門檻**要改跟 Leo 講一聲，不要自己動**（不自行變更投資判定門檻）。
# ⚠️ 樣本 433 天不含完整空頭循環、未扣交易成本——所以只當警示不當指令。
WARN_N = 7
...
def drawdown_warning(st=None):
    ...
    if st.get("streak", 0) < WARN_N:
        return None
    return (f"　⚠️ 已連 {st['streak']} 日低於均線（≥{WARN_N} 日）。"
            f"回測（433 個交易日）顯示這種狀態下**最大回檔 -26.7%**，"
            f"而連續 {WARN_N} 日內轉為空手可把回檔壓到 **-10.4%**、報酬幾乎不變。"
            f"⚠️ 這是**回檔警示不是出場指令**——樣本不含完整空頭循環、未扣交易成本，"
            f"要不要動作是你的決定。")
