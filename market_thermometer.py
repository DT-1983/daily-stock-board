...
def drawdown_warning(st=None):
    ...
    if WARN_N <= 7:
        ...
        # 2026-09-09 Leo 指定拿掉這段（原本是 N≥8 沒有回測支持的完整說明：
        # 433 個交易日的報酬與最大回檔對照、樣本不含空頭循環等）。
        # 理由是它每天重複同一段長文字，佔掉日報一大塊版面。
        # ⚠️ 但**不能完全消音**：N=32 這個設定在回測裡確實沒有減少回檔的證據，
        #    完全不提就變成「看起來像有依據的訊號」。留最短的一句話當標記，
        #    完整依據在 memory/market_thermometer_ef_ratio.md。
        why = "（N=32 無回測支持，僅狀態標記）"
